import {
  ...
  buildPersistedPayload
    ...
    /** index.html 옆 ./assets/ — 배포·다른 PC에서도 동작. portrait-0.png … portrait-5.png */
    ...
    const CHARACTER_IMAGE_PLACEHOLDER =
      "data:image/svg+xml," + encodeURIComponent(
        '<svg xmlns="http://www.w3.org/2000/svg" width="400" height="560"><rect width="100%" height="100%" fill="#1a2734"/><text x="50%" y="46%" fill="#d1d1d6" font-size="14" font-family="sans-serif" text-anchor="middle">캐릭터 이미지 없음</text><text x="50%" y="54%" fill="#9aa3b0" font-size="12" font-family="sans-serif" text-anchor="middle">./assets/portrait-0.png ~ portrait-5.png</text></svg>'
      );

    function getCharacterImageSrc() {
      const path = (i) => CHARACTER_IMAGE_BASE + CHARACTER_IMAGE_FILES[i];
      if (state.endingId === 1) return path(3);
      if (state.endingId === 2) return path(4);
      if (state.endingId === 3) return path(5);
      const a = state.affection;
      if (a <= 30) return path(0);
      if (a <= 50) return path(1);
      return path(2);
    }

    function getCharacterImageAlt() {
      if (state.endingId === 1) return "인안나 · 청혼 엔딩";
      if (state.endingId === 2) return "인안나 · 얀데레 엔딩";
      if (state.endingId === 3) return "인안나 · 유학 엔딩";
      const a = state.affection;
      if (a <= 30) return "인안나 · 호감도 0~30";
      if (a <= 50) return "인안나 · 호감도 31~50";
      return "인안나 · 호감도 51~엔딩 직전";
    }
    ...
    function getCharacterImageAltByIndex(fileIndex) {
      return "인안나 · " + (PORTRAIT_STAGE_LABELS[fileIndex] || "");
    }
    ...
    function mergeRomanceNarrativePatch(raw) {
      ...
      if (!out.postEndingDaily || typeof out.postEndingDaily !== "object") out.postEndingDaily = d.postEndingDaily;
    ...
    function buildTogetherStudyLineForRecord(rec) {
      const tpls = romanceNarrative.togetherLineTemplates || [];
      const t = pickFromArr(tpls, (rec.date || "") + "_" + rec.seconds + "_" + (rec.tag || ""));
      const min = Math.max(1, Math.round(Number(rec.seconds || 0) / 60));
      const tag = (rec.tag && String(rec.tag).trim()) ? String(rec.tag).trim() : "공부";
      return t.replace(/\{min\}/g, String(min)).replace(/\{tag\}/g, tag);
    }
    ...
    function drawRhythmHeatmap(now) {
      ...
      const light = document.documentElement.getAttribute("data-a11y") === "light";
      const cellFill = light ? "rgba(89, 66, 110, 0.22)" : "rgba(114, 15, 50, 0.35)";
      const cellHi = light ? "rgba(89, 66, 110, 0.55)" : "rgba(114, 15, 50, 0.78)";
      const grid = light ? "rgba(49, 44, 50, 0.12)" : "rgba(209, 209, 214, 0.1)";
      const label = light ? "#312c32" : "rgba(209, 209, 214, 0.85)";
    ...
    function sanitizeMemoryMoment(m) {
    ...
    function buildStorySummaryHtml() {
      ...
      const cnt = { milestone: 0, interaction: 0, choice: 0, ending: 0, userReaction: 0, branch: 0, dateScene: 0, studyTogether: 0, other: 0 };
      log.forEach((e) => {
        const t = e.type;
    ...
    function gatherPersistedState() {
      return buildPersistedPayload(state);
    }
    ...
    function hydrateStateFromPlainObject(data) {
      ...
      state.a11yPreset = data.a11yPreset === "strong" || data.a11yPreset === "light" ? data.a11yPreset : "soft";
    ...
    function loadState() {
      ...
      try {
        ...
        migrateStoredRoot(parsed);
        hydrateStateFromPlainObject(parsed);
      } catch (_) {}
    }
    ...
    function applyA11yPresetToDocument() {
      const p = state.a11yPreset === "strong" || state.a11yPreset === "light" ? state.a11yPreset : "soft";
      state.a11yPreset = p;
      document.documentElement.setAttribute("data-a11y", p);
    }
    ...
    function drawBarChart(canvasId, series, colorA, colorB) {
      ...
      const light = document.documentElement.getAttribute("data-a11y") === "light";
      const barA = light ? "#59426e" : colorA;
      const barB = light ? "#8c8061" : colorB;
      const gridStroke = light ? "rgba(49, 44, 50, 0.1)" : "rgba(209, 209, 214, 0.12)";
      const labelFill = light ? "#312c32" : "rgba(209, 209, 214, 0.88)";
    ...
    function focusFirstInMainPanel(tabId) {
      const root = tabId === "record" ? $("record") : tabId === "view" ? $("view") : null;
      if (!root) return;
    ...
    function render() {
      ...
      $("userNameViewEdit").value = state.userName || "";
      $("goalHours").value = state.goalHours || "";
      $("questText").value = state.quest || "";
      $("focusMinutes").value = state.focusMinutes;
      $("breakMinutes").value = state.breakMinutes;
      $("autoViewSwitch").checked = state.autoSwitchToView;
      $("goalQuestStatus").textContent = state.goalHours
      ...
      const a11ySel = $("a11yPresetSelect");
      if (a11ySel) a11ySel.value = state.a11yPreset || "soft";
      ...
      $("todayTotal").textContent = Math.round(sumSecondsByDate(today) / 60) + "분";
      $("allTotal").textContent = fmtHourMin(state.totalSeconds);
      $("affectionDisplay").textContent = String(state.affection);
      ...
      if (state.endingId) {
        ...
        $("romanceStage").textContent = st;
        if (vnStageEl) vnStageEl.textContent = st;
        $("dialogueBox").textContent = getEndingMainDialogue(displayName, voc);
        $("nextStageInfo").textContent = "-";
      } else {
        $("romanceStage").textContent = nowDialogue.stage;
        if (vnStageEl) vnStageEl.textContent = nowDialogue.stage;
        $("dialogueBox").textContent = buildMainDialogueLine(displayName, voc, nowDialogue.text);
        $("nextStageInfo").textContent = nextDialogue
          ? nextDialogue.targetStage + "까지 " + nextDialogue.need + " 필요"
          : "최고 단계 달성";
      }
      const relMeta = $("relationshipMetaLine");
      if (relMeta) relMeta.textContent = state.endingId ? "" : getRelationshipMetaLine();
      ...
      $("heartMeter").textContent = buildHeartText();
      const routeEl = $("routeMeter");
      if (routeEl) routeEl.innerHTML = buildRouteMeterHtml();
      $("affectionSummary").textContent = displayName + " · 인안나와의 호감도 " + state.affection + " · "
        + (state.endingId
          ? (state.endingId === 1 ? "엔딩 · 청혼" : state.endingId === 2 ? "엔딩 · 얀데레" : "엔딩 · 유학 제안")
          : nowDialogue.stage);
      if (state.endingId) {
        $("endingPanel").style.display = "block";
        $("endingTitle").textContent = getEndingTitle(state.endingId);
        $("endingBody").textContent = getEndingBody(state.endingId, displayName, voc);
      } else {
      ...
      $("todaySessionCount").textContent = "오늘 포모도로 세션 " + getTodayPomodoroSessions() + "회";
      const dToday = $("dockTodayShort");
      if (dToday) dToday.textContent = Math.round(sumSecondsByDate(today) / 60) + "분";
      $("soundToggleBtn").textContent = "알림음: " + (state.soundEnabled ? "켜짐" : "꺼짐");
      if (state.activeInteraction && !state.endingId) {
        $("talkBox").style.display = "block";
        $("npcTalkLine").textContent = "인안나: " + state.activeInteraction.npc;
        $("replyChoices").innerHTML = state.activeInteraction.choices
          ...
          .join("");
        $("replyResult").textContent = state.lastReplyText;
      } else {
    ...
    function setupTabs() {
      ...
      buttons.forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          render();
          focusFirstInMainPanel(btn.dataset.tab || "record");
        });
      });
    }
    ...
    function setupActions() {
      ...
      $("saveGoalQuestBtn").addEventListener("click", () => {
        state.goalHours = Number($("goalHours").value || 0);
        state.weeklyGoalHours = Math.max(0, Number($("weeklyGoalHours").value || 0));
        state.quest = $("questText").value.trim();
      ...
      const a11yPresetSelect = $("a11yPresetSelect");
      if (a11yPresetSelect) {
        a11yPresetSelect.addEventListener("change", () => {
          state.a11yPreset = a11yPresetSelect.value || "soft";
          applyA11yPresetToDocument();
          saveState();
          render();
        });
        ...
        pomoNotifyBtn.addEventListener("click", () => {
          ...
          const ask = () => {
            Notification.requestPermission().then((perm) => {
              ...
              render();
              try {
                new Notification("인안나와 공부", { body: "포모 단계가 바뀔 때 알려 드릴게요." });
              } catch (_) {}
            });
          };
          if (Notification.permission === "default") {
            ...
            render();
            try {
              new Notification("인안나와 공부", { body: "포모 알림이 켜졌어요." });
            } catch (_) {}
          }
        });
      }
      ...
      $("userNameViewEdit").addEventListener("input", () => {
        state.userName = $("userNameViewEdit").value.trim();
        saveState();
        render();
      });
      ...
      $("startPomoBtn").addEventListener("click", () => {
        ...
        const rest = Number($("breakMinutes").value);
      ...
      $("autoViewSwitch").addEventListener("change", () => {
        state.autoSwitchToView = $("autoViewSwitch").checked;
        saveState();
      });
      ...
      $("replyChoices").addEventListener("click", (event) => {
        ...
        state.replyAffectionBonus += selected.delta;
        applyRoutePoints(selected);
        recalcAffectionTotal();
        ...
        const rp = selected.routePts || 3;
        state.lastReplyText = displayName + voc + ": " + selected.text
          + "  |  호감도 " + sign + selected.delta + " · " + routeLabel + " 루트 +" + rp;
        appendStoryLog({
          type: "choice",
          title: "대화 기록 · 선택지",
      ...
      $("historyList").addEventListener("click", (event) => {
        ...
        const dateInputEl = $("historyList").querySelector("input[data-field='date'][data-edit-index='" + recordIndex + "']");
        const minutesInputEl = $("historyList").querySelector("input[data-field='minutes'][data-edit-index='" + recordIndex + "']");
        const tagInputEl = $("historyList").querySelector("input[data-field='tag'][data-edit-index='" + recordIndex + "']");
    ...
    const viewSubTabsEl = document.querySelector(".view-sub-tabs");
    if (viewSubTabsEl) {
      viewSubTabsEl.addEventListener("click", (event) => {
        const btn = event.target && event.target.closest ? event.target.closest("[data-view-sub]") : null;
        if (!btn) return;
